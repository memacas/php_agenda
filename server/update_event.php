    ...
    if (isset($_POST['id'])){
      ...
      $eventos =  $baseDatos->consultaToArray(array('paramConsulta' => array('tablas' => array('eventos'),
                                                                            ...
                                                                            'condicion' => $_SESSION['id_usuario'] . '= id_usuario AND
                                                                                           id = ' . $_POST['id'])));
      //Actualiza el registro en caso de que exista
      if ($eventos['numRows'] > 0) {
        $evento = array();
        $evento['start_date'] = '"' . $_POST['start_date'] . '"';
        $evento['end_date'] = (!isset($_POST['end_date']))?'"' . $_POST['start_date'] . '"':'"' . $_POST['end_date'] . '"';

        $baseDatos->actualizarRegistro('eventos', $evento, $_SESSION['id_usuario'] . '= id_usuario AND id = ' . $_POST['id']);
      }
      else $retorno = array('msg' => 'NO HAY EVENTO A ACTUALIZAR');
